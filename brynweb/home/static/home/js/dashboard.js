(function (bryn, $, undefined) {
  ...
  dashboard.ajaxInstanceAction = function (event, url) {
    ...
    $.ajax({
      ...
      complete: function (xhr) {
        ...
        dashboard.refreshInstancesAfter(tableContainer, [
          1000,
          2000,
          5000,
          10000,
  ...
  */
  ...
  $(document).on(
    "submit",
    "form.launch-custom-form, form.launch-gvl-form",
    function (event) {
      event.preventDefault();
      bryn.ajaxForms.formPOST(this, true, function (form) {
        var tableContainer = $(form)
          .closest(".team-container")
          .find(".instances-table-container");
        $("html, body")
          .stop()
          .animate({ scrollTop: tableContainer.offset().top }, "500", "swing");
  ...
  // Instance actions
  $(document).on("click", ".instance-actions-list > a", function (event) {
    event.preventDefault();
    var anchor = $(this);
  ...
  $(document).ready(function () {
    ...
    $("form.launch-custom-form").each(function () {
      var form = $(this);
      if (
        form.find("select[name='server_key_name_choice']").val() != "bryn:new"
      ) {
    ...
    $("form.launch-custom-form select[name='server_key_name_choice']").change(
      function () {
        var keySelect = $(this);
