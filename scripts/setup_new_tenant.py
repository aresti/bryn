...
t = keystone.tenants.create(
    tenant_name=tenant_name, description="bryn: test create tenant", enabled=True
)
keystone.users.create(name="bryn:test", password="bryn:test", tenant_id=t.id)

neutron = client.get_neutron()

network = {"tenant_id": t.id, "name": "tenant1-private", "admin_state_up": True}
n = neutron.create_network({"network": network})

router = {"tenant_id": t.id, "name": "tenant1-router", "admin_state_up": True}
...
neutron subnet-create  tenant1-private 192.168.0.0/24 --tenant-id 6847328801474250af62e4cbfb7b661f --name
    tenant1-192.168.0.0/24" --gateway 192.168.0.1  --allocation-pool start=192.168.0.50,end=192.168.0.200 --enable-dhcp
    --ip-version 4
